...
---
    ...
    When making requests, please note the following:
      - **Query Efficiency:** Be considerate of system resources and avoid making unoptimized requests. We reserve the right to terminate API access if your average request latency exceeds 1 second over the course of 5 minutes.
    ...
    # Changelog
    + 2026-06-29
        + add `practitionerId` query parameter on `appointment` endpoint to filter appointments by practitionerId.
        + add `practitionerId` query parameter on `busy-time` endpoint to filter busy times by practitionerId.
        
    + 2026-04-22
        + add `connections` into practitioner response.
        + add `associatedServiceIds` into location response.
